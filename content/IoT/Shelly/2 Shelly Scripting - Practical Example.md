...
> [!tip] I didn't end up using `aenergy` notifications, covering here for reference.
...
> [!tip] The script below uses the `apower` notifications, these alone are sufficient to determine "is the load idle". The only slight hiccup is that these notifications do not include a timestamp so that needs to be tracked separately.
