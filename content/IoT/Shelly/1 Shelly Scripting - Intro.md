...
> [!tip] [`var` vs `let`](https://www.google.com/search?q=javascript+let+var)
> Though nominally only available in ES6, [Espruino parses](https://www.espruino.com/Features) `let` however treats it as `var`. The two have different behaviours in ES6, regarding scoping of the declared variables. It seems sensible to use (only) `var` to avoid misleading behaviour (`let` actually behaving as `var`) and/or avoid bugs if/when Shelly moves on to ES6.
...
The script below is contrived, intended to demonstrate both a common mistaken approach to Shelly's asynchronous environment, and a better way. The goal of the script is to periodically change the value of a global variable: increment the value if the current time is less than the :30s mark, otherwise decrement the variable.

An unsophisticated approach that seems fairly common on the Shelly forums is to do everything in layers of anonymous function callbacks. We'll start with that.

In the first iteration below, the Shelly [`Timer`](https://shelly-api-docs.shelly.cloud/gen2/Scripts/ShellyScriptLanguageFeatures/#timer) object is used to create a periodic timer; `Timer.Set` will call the provided callback function when the timer goes off. In this first example, the callback is an anonymous function: it's provided as the third agument in the Timer's argument list and has no name.

The first anonymous function invokes the [`Sys.GetStatus`](https://shelly-api-docs.shelly.cloud/gen2/ComponentsAndServices/Sys#sysgetstatus) RPC call, which returns the current time in seconds (since 1970, aka `unixtime`). That invocation via `Shelly.call` expects a callback and again an anonymous function is used. This anonymous function copies the RPC result's `unixtime` field over to the `unixtime` global variable (global variables always being in scope).
...
Note the `unixtime%60`, which is the 00-59 seconds component of current time, lags behind the actual time (and the value on the first call is 0, the initial value when declared). What's going on here is a race condition: `Shelly.call` is asynchronous, that is the script executes that function - dutifully firing off an RPC call - and then immediately carries on with the next instruction, in this case the `if (unixtime%60 < 30)` statement. The actual RPC call (likely) hasn't finished yet and the anonymous callback has not been called, and thus the result not yet copied over to the global `unixtime`. The asynchronous `Shelly.call` does not wait for the RPC to complete, it's "fire and forget".
...
The 'take 2' script chains two asynchronous calls: the anonymous function callback from `Timer.set()` does nothing else but set up a recurring invocation of `Shelly.call('Sys.GetStatus')` with a callback to `getStatusCallback()` to do "the actual work". The setup is only ever called once when the script starts and every time the callback's invoked it is passed the device `Status`, from which it extracts the current time.

This is all well and good (i.e. no [busy waits](https://en.wikipedia.org/wiki/Busy_waiting)!), though as it happens for this use case Shelly provide a synchronous (not RPC) API interface to get the device's time: [`Shelly.getComponentStatus('Sys')`](https://shelly-api-docs.shelly.cloud/gen2/Scripts/ShellyScriptLanguageFeatures#shellygetcomponentstatus). Replacing the asynchronous `Sys.GetStatus` RPC call with the synchronous API call simplifies things markedly.

We should also handle the situation where the clock [may not yet be synced](https://shelly-api-docs.shelly.cloud/gen2/ComponentsAndServices/Sys#status). Note that if you didn't care about alignment with wall clock time the `uptime` field would be a better option as that doesn't rely on NTP and is always valid.
...
#### Don't Get Caught Out When - Not If - Things Fail

Regarding the test for `unixtime`: it's always a good idea to test return values from Shelly calls for validity first before charging ahead and attempting to use return object values; e.g. `Shelly.getComponentStatus("switch", 0)` will return an object when all is well and good but can return other values (e.g. `null` if the switch index is out of range for the device). Recall that Shelly will kill your script on exceptions including attempting to dereference a property that doesn't exist ('Uncaught ReferenceError'). Robust code assumes as little as possible.

Whilst in the above situation it's sufficient to simply test if `unixtime` is "true", as all the "false" values are either Shelly errors or an invalid timestamp (`null`, `undefined` and `<= 0`). More generally separate tests for "Shelly error" and "is the return value as expected" are best (clearer on intention at the least).
...
function init() {
  if (CONFIG.log) {
    // set up the log timer; this burns a relatively precious resource but could be moved to
    // piggyback on an existing timer callback
    Timer.set(_logQueue.interval, true, _logWrite);
  }
  _log("init");
  for (const i in CONFIG.timers) {
...
[[2 Shelly Scripting - Practical Example|Part 2]] covers a practical / real-world example.
